CREATE OR REPLACE PACKAGE BODY pkwf_99_eml_itf
AS
  --
  /* Created By       : Chiedu Okeleke
  ** Date Created     : 27-JAN-2013
  **
  ** Cloned from SDSEMAIL by Sean D. Stuber, sean.stuber@gmail.com  
  ** http://www.experts-exchange.com/Database/Oracle/A_7749-How-to-send-email-attachments-with-Oracle.html
  **
  ** Release Version  : 2.0
  **
  ** PURPOSE: Email STMP Server Interface
  **
  ** MODIFICATION HISTORY:
  **
  **    Modified By   : Femi Akintoye
  **    Date          : 08-feb-2015
  **    Version       :
  **    Remarks       : Made some modifications to allow direct use to google
  **                  : http://www.oracleatoz.com/how-to-send-email-in-oracle-using-gmail
  ...
  **    Remarks       :
  **  
  ** Copyright:  Neulogic Solutions, 2013
  **
  */
  --
    ...
    */
    g_default_smtp_server         VARCHAR2(100) := NULL;
    g_default_smtp_port           INTEGER := 25;
    --
    g_raise_error                 BOOLEAN := TRUE;
    g_smtp_authentication         BOOLEAN := FALSE;
    g_default_smtp_usr            VARCHAR2(100) := NULL;
    g_default_smtp_pwd            VARCHAR2(100) := NULL;
    --
    c_default_timeout             CONSTANT INTEGER := 60; -- 60 seconds
    --
    ...
    ---------------------------------------------------------------------------
    --
    FUNCTION find_a_server(p_server_list IN VARCHAR2 DEFAULT NULL) RETURN connection IS
    ...
    BEGIN   
      ...
      THEN
        IF g_default_smtp_server IS NULL
        THEN  
         SELECT *
           INTO v_servers
          ...
          WHERE ROWNUM = 1;
        ELSE
          v_servers := g_default_smtp_server || ':' || g_default_smtp_port;
        END IF;
      ELSE
      ...
      LOOP
        ...
        BEGIN    
          ...
          v_connection := UTL_SMTP.open_connection(v_host, v_port, c_default_timeout);
--                UTL_SMTP.helo(v_connection, v_host);
--                Had to change this line as the SMTP Server didnt advetise auth requirements
--                May need to make this an email parameter as there is no standard
--                 Each SMTP implements its own policy. Introduce a flag to determine which to use
            UTL_SMTP.ehlo(v_connection, v_host);
          IF g_smtp_authentication THEN
            UTL_SMTP.command(v_connection, 'AUTH LOGIN');
            UTL_SMTP.command(v_connection, utl_raw.cast_to_varchar2( utl_encode.base64_encode( utl_raw.cast_to_raw( g_default_smtp_usr ))) );
            UTL_SMTP.command(v_connection, utl_raw.cast_to_varchar2( utl_encode.base64_encode( utl_raw.cast_to_raw( g_default_smtp_pwd ))) );                 
          END IF;
          v_connected := TRUE;
        EXCEPTION
          WHEN OTHERS
    ...
    PROCEDURE parse_email_list(p_addresses IN VARCHAR2, p_list IN OUT NOCOPY DBMS_SQL.varchar2a)
    IS
    ...
    IS
      v_connection   UTL_SMTP.connection;
      v_address_list DBMS_SQL.varchar2a; 
      v_reply        UTL_SMTP.reply;
    BEGIN   
      v_connection := find_a_server(p_server_list);
      --
      UTL_SMTP.COMMAND(v_connection, 'MAIL FROM: <'||extract_address(p_sender)||'>');
--@@@@@   v_reply := UTL_SMTP.mail(v_connection, extract_address(p_sender));
      write_to_log('Mail(' || p_sender || '): ' || TO_CHAR(v_reply.code, 'fm009') || ' ' || v_reply.text);
      --
      ...
      THEN
        ...
        LOOP
          UTL_SMTP.COMMAND(v_connection, 'RCPT TO: <'||v_address_list(i)||'>');
--@@@@@           v_reply := UTL_SMTP.rcpt(v_connection, v_address_list(i));
      ...
      END IF;
      --
      IF p_clob IS NOT NULL OR p_blob IS NOT NULL THEN
        ...
        UTL_SMTP.write_data(v_connection, 'This is a multi-part message in MIME format.' || c_crlf
        );
        --
    ...
    EXCEPTION
      WHEN OTHERS THEN
        -- logger.log('ERROR OCCURED DURING SEND eml_itf.send_all >'||dbms_utility.format_error_stack,'eml');
    ...
    PROCEDURE set_smtp_server_params(
        p_host         IN           VARCHAR2,
        p_port         IN           INTEGER,
        p_auth         IN           BOOLEAN DEFAULT FALSE,
        p_usr          IN           VARCHAR2 DEFAULT NULL,
        p_pwd          IN           VARCHAR2 DEFAULT NULL,
        p_raise_err    IN           BOOLEAN DEFAULT TRUE
    ) 
    IS
    BEGIN    
      g_default_smtp_server  := p_host;
      g_default_smtp_port    := p_port;
      g_smtp_authentication  := p_auth;
      g_default_smtp_usr     := p_usr;
      g_default_smtp_pwd     := p_pwd;   
      g_raise_error          := p_raise_err;      
    END set_smtp_server_params;
    --
BEGIN
